...
def _brief_prompt(items: List[Dict[str, Any]]) -> str:
    rows = []
    for it in items:
        ts = it.get("ts")
        rows.append({
            ...
            "시각": ts.strftime("%H:%M") if isinstance(ts, datetime) else "",
            "내용": it.get("body") or "",
        })
...
def _summarize_pending(pending: List[Dict[str, Any]], now: datetime) -> tuple:
    ...
    if pending:
        ...
        if alias:
            try:
                r = llm.call(alias, _brief_prompt(pending), system=SIGNALS_SYSTEM,
                             expect_json=True)
                items = _parse_items(

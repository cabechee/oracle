...
카테고리:
- action_needed: 회신·결제·예약확인·기한 등 **사용자가 당장 뭔가 해야** 하는 것
- attention: 사용자가 챙길 만한, '내 일'에 직접 닿는 정보 — 내가 한 주문의 배송·결제, 내 일정 변경, 잔액·예약 확정 등. (급하진 않지만 알아두면 좋은 것)
- acquaintance: 지인·개인이 보낸 연락 (개인 번호 010 등에서 온 대화체)
- low: 루틴·자동 알림 — 시스템 메시지, 영수증성 자동안내, 앱의 일반 푸시 등 **굳이 신경 안 써도 되는 것**. (attention과 애매하면 low로 — 과하게 '관심'으로 올리지 말 것)
- spam: 광고·홍보·마케팅
...
def _brief_prompt(items: List[Dict[str, Any]], now: Optional[datetime] = None) -> str:
    ...
    prefs = _sender_prefs()
    pf = (f"\n\n[사용자 지정 카테고리 — 아래 발신자는 사용자가 직접 이 카테고리로 정했다. "
          f"그대로 따르되, 내용이 명백히 당장 행동 필요면(action_needed)만 예외]\n"
          f"{json.dumps(prefs, ensure_ascii=False)}") if prefs else ""
    return (
...
# ── 발신자별 카테고리 학습 — 재분류하면 그 발신자 취향이 눌러붙는다 ──
def _sender_prefs() -> Dict[str, str]:
    """발신자별 사용자 지정 카테고리. {보낸이: category}. '이 발신자는 늘 관심없음/관심' 취향."""
    try:
        doc = db.settings().find_one({"_id": "signal_sender_prefs"}) or {}
    except Exception:
        return {}
    return {k: v for k, v in doc.items()
            if k != "_id" and isinstance(v, str) and v in SIGNAL_CATEGORIES}


def _save_sender_pref(sender: str, category: str) -> None:
    s = (sender or "").strip()
    if s and s != "(알 수 없음)" and category in SIGNAL_CATEGORIES:
        db.settings().update_one({"_id": "signal_sender_prefs"},
                                 {"$set": {s: category}}, upsert=True)


def _apply_sender_prefs(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """사용자 지정 발신자 카테고리 적용 — 단 LLM이 action_needed로 본 건 유지(긴급은 발신자
    무관하게 띄움). 비-긴급만 사용자 지정대로 눌러붙는다."""
    prefs = _sender_prefs()
    if not prefs:
        return items
    for it in items:
        pref = prefs.get((it.get("sender") or "").strip())
        if pref and it.get("category") != "action_needed":
            it["category"] = pref
    return items
    ...
    if pending:
        ...
        if alias:
            try:
                ...
                items = _apply_sender_prefs(items)   # 사용자 지정 발신자 카테고리 눌러붙임
...
def recategorize(signal_ids: List[str], category: str) -> int:
    ...
    if not sids:
        return 0
    n = 0
    learned: set = set()        # 이 발신자들은 앞으로 이 카테고리로 (학습)
    for b in db.signal_briefs().find({"items.signal_ids": {"$in": list(sids)}}):
        ...
        if changed:
            ...
            n += 1
    for snd in learned:         # 다음부터 같은 발신자는 이 카테고리로 눌러붙음
        _save_sender_pref(snd, category)
    return n

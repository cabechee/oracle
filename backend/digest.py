...
마크다운 본문(상위 헤더는 호출자가 # 날짜로 붙임). 친근하고 간결한 한국어. 4~8문장 또는 짧은 리스트.
...
def run_daily(target_date: Optional[date] = None) -> Dict[str, Any]:
    ...
    # 1) thread 부착
    thread_res = _judge_threads(records) or {}
    ...
    # 2) type + tags 부착
    type_res = _classify_type_tags(records) or {}
    ...
    # 3) daily digest 마크다운 → vault digest/
    digest_text = _make_digest(records, target)
...
def _make_digest(records: List[Dict[str, Any]], target: date) -> str:
    ...
    prompt = (
        ...
        f"{json.dumps(_records_brief(records), ensure_ascii=False, indent=2)}\n\n"
        f"위 기록들을 보고 오늘의 다이제스트를 작성."
    )
    try:
